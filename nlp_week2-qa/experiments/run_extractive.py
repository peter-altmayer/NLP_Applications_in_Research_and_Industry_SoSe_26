...
Runs deepset/roberta-base-squad2 and deepset/electra-base-squad2
...
from src.model import run_extractive
...
def main():
    ...
    for short_name, model_name in MODELS.items():
        ...
        results = run_extractive(model_name, examples)
        df = pd.DataFrame(results)

        path = RESULTS_DIR / f"extractive_{short_name}.csv"
    ...
    merged.to_csv(RESULTS_DIR / "extractive_all.csv", index=False)
    print(f"\nMerged results saved to {RESULTS_DIR / 'extractive_all.csv'}")
