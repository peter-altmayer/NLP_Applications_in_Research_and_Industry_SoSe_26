...
from src.retriever import BM25Retriever, DenseRetriever, OracleRetriever, compute_retrieval_metrics
...
def run_eval(
    ...
    for s in tqdm(samples, desc="Evaluating"):
        ...
        # Retrieve — oracle uses gold snippets directly
        if retrieval == "oracle":
            retrieved = retriever.retrieve(question, k=k, snippets=relevant)
        else:
        ...
        # Predict — pass answer_type for routing in DualHeadQAModel (Task 6)
        pred = model.predict(question, context, answer_type=s.get("answer_type"))
...
ALL_RETRIEVALS = ["bm25", "dense"]           # used by --all (no oracle, to keep runtime predictable)
ALL_RETRIEVALS_CLI = ALL_RETRIEVALS + ["oracle"]
...
def main() -> None:
    ...
    parser.add_argument("--retrieval", choices=ALL_RETRIEVALS_CLI, default="dense")
    ...
    parser.add_argument("--all", action="store_true", help="Run all model×dataset×retrieval combos (bm25+dense)")
    parser.add_argument("--all-oracle", action="store_true", help="Run oracle retrieval for all models × datasets")
    ...
    if args.all_oracle:
        for model in ALL_MODELS:
            for dataset in ALL_DATASETS:
                try:
                    run_eval(model, dataset, "oracle", args.k, args.n, args.skip_faithfulness)
                except FileNotFoundError as e:
                    print(f"  SKIP {model}/{dataset}/oracle: {e}")
        return
    ...
    if not args.model or not args.dataset:
        parser.error("--model and --dataset are required (or use --all / --all-oracle / --fine-tune)")
